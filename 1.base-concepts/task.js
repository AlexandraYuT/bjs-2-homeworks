...
function calculateTotalMortgage(percent, contribution, amount, countMonths) {
	...
	if (isNaN(per) || per < 0) {
		...
		let today = new Date();
		if (today.getFullYear() > countMonths.getFullYear() || today.getMonth() > date.getMonth()) {
			return `Параметр "срок ипотеки" содержит неправильное значение ${countMonths}`;
		} else {
			let s = am - con; //тело кредита
			let n = countMonths.getMonth() - today.getMonth() + (12 * (date.getFullYear() - today.getFullYear())); //срок кредита в месяцах
